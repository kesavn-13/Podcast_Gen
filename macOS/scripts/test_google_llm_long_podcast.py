...
async def test_google_llm_and_create_podcast():
    ...
    try:
        ...
        # 5. Create enhanced audio system (Coqui-only)
        print("🎤 Initializing enhanced audio system (Coqui Local)...")
        
        try:
            tts_engine, podcast_producer = create_enhanced_audio_system()
            print("✅ Audio system ready (Coqui Local)")
        except Exception as e:
            print(f"⚠️  Audio system initialization failed: {e}")
            print("   This is expected if audio libraries aren't installed yet")
            print("   Install with: pip install TTS pydub")
            return
        ...
        print("✅ Enhanced audio pipeline: READY (Coqui)")
        print("\n📋 Next Steps:")
        print("   1. Install audio dependencies: pip install TTS pydub")
